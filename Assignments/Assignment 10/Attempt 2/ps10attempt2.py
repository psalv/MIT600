...
points = buildCountyPoints('/Users/paulsalvatore57/PycharmProjects/MIT600/Assignments/Assignment 10/Attempt 2/counties.txt')
# random.seed(123)
testPoints = random.sample(points, len(points)/10)

part_points = randomPartition(points, 0.8)  #[0] is test, [1] is holdout
# print len(part_points[0])
# print len(part_points[1])


def find_save_error(training, holdout, name):
    """
    ...
    for k in kvals:
        cluster_distances = []
        print k
        clusters = kmeans(training, k, 0.1, County)[0]
        for cluster in clusters:
            cl_points = cluster.getPoints()
            for point in cl_points:
                cluster_distances.append(point.distance(cluster.getCentroid()))
        allCluster_distances.append(cluster_distances)

        holdout_distances = []
        for point in holdout:
            shortest = None
            for cluster in clusters:
                dist = (point.distance(cluster.getCentroid()))
                if shortest == None:
                    shortest = dist
                if dist < shortest:
                    shortest = dist
            holdout_distances.append(shortest)

        er2 = 0
        for i in holdout_distances:
            er2 += i**2
        er2 = er2/len(holdout_distances)
        error[1].append(er2)

    for i in allCluster_distances:
        er = 0
        for j in i:
            er += j**2
        er = er/len(i)
...
find_save_error(part_points[0], part_points[1], 'Q1 Error Values, training and holdout')
# holdout = pickle.load(open('Q1 Error Values, training and holdout'))
# print holdout

#What I need to do now is iterate through each point of the holdout set and find which centroid is closest to the point.
#After finding the closest point I will get the distance of the point to the centroid, and then calculate error as I did before.
#This must be done for each k-value, therefore must be within the for loop.
...
def graphRemovedErr():
    ...
    errors = pickle.load(open('Q1 Error Values, training and holdout'))

    kvals = [25, 50, 75, 100, 125, 150]

    pylab.plot(kvals, errors[1], label = 'Holdout')
    pylab.plot(kvals, errors[0], label = 'Training')
    pylab.title('Q1 Error Values, holdout and training')
    pylab.xlabel('K value')
    pylab.ylabel('Total Error')
    pylab.legend(loc = 'best')
    pylab.savefig('Q1 Total Error Attempt 2')
    pylab.show()
...
#I'm not going to do the rest, that is more or less contained in attempt one.